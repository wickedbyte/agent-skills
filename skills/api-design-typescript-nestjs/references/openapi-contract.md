...
1. **Emitted ≡ canonical** — an in-repo test asserts the document the service _serves_ at `/openapi.json` equals the
   frozen `openapi.yaml`.
...
canonical _by construction_ — there is no generator to tune — and a drift check keeps the embedded copy honest.
...
When the service _defines_ the API, generate the document from the same schemas you validate with, so the doc can't
...
const canonical = parse(
    readFileSync(resolve(here, "../../../openapi.yaml"), "utf8"),
...
The in-repo test proves the _document_ is right; the fuzzer proves the _server_ obeys it. Schemathesis reads the spec,
...
the spec doesn't declare. A common, _correct_ outcome to understand: a fuzzer run against a **non-persistent mock** will
