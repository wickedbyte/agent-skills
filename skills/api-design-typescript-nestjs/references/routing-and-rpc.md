...
describe("splitColonCommand", () => {
    it("splits on the last colon", () => {
        expect(splitColonCommand("task_01J:complete")).toEqual({
            id: "task_01J",
            command: "complete",
        });
    });
    it("treats a bare id as command-less", () => {
        expect(splitColonCommand("task_01J")).toEqual({
            id: "task_01J",
            command: null,
        });
    });
    it("keeps a colon inside the id, taking only the trailing command", () => {
        expect(splitColonCommand("a:b:complete")).toEqual({
            id: "a:b",
            command: "complete",
        });
    });
...
it("a known command on a missing task → 404 not_found", async () => {
    const res = await request(server)
        .post("/tasks/task_missing:complete")
        .send({});
