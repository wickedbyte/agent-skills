...
export async function migrateToLatest(db: Kysely<Database>): Promise<void> {
    const { error, results } = await db /* via Migrator */ .migrateToLatest();
...
await sql`CREATE INDEX events_stream_idx ON events (stream_id, version)`.execute(db);
...
try {
    ...
    if (isUniqueViolation(err)) throw new VersionConflictError("stream advanced concurrently");
    throw err;
}
```
