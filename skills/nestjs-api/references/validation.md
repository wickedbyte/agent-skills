...
```ts
export type Patch<T> = { readonly set: false } | { readonly set: true; readonly value: T };
...
- **Semantic / cross-field** validation belongs in the **domain** (`decide`), for two reasons: the error's `field` and
  ...
  need the *resolved* values (after merging a partial update against current state), which the boundary doesn't have.
...
*parsing* in the domain (`parseIsoDate`) also means one canonical parser, not a Zod regex that drifts from it.
