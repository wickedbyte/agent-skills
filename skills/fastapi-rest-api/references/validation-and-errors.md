...
- **`Field` constraints** (`min_length`, `ge`, `pattern`, …) are validation *and* schema. Prefer them over manual checks.
...
```json
{ "error": { "code": "validation_failed", "message": "…", "details": { "field": "dueDate", "reason": "…" } } }
```
...
its *own* body shape: `{"detail": [{"loc": [...], "msg": "...", "type": "..."}]}`. That is not your envelope. Install the
...
**The schema.** FastAPI also *documents* a generated `HTTPValidationError` schema for every operation's 422. If your
...
deliberately generic `internal_error` (never echo internals to the client), and make sure every *expected* failure has
